package com.tekinarslan.material.sample.view;
...
public class BarrageView extends RelativeLayout {
    ...
    private BarrageHandler mHandler = new BarrageHandler();//弹幕消息处理
    private Random random = new Random(System.currentTimeMillis());//随机数
...
//    private List<BarrageItem> itemList = new ArrayList<BarrageItem>();
    ...
    //弹幕持续时间
        ...
        mHandler.sendEmptyMessageDelayed(0, duration);//发送消息
    }
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
        ...
        totalHeight = getMeasuredHeight();//获取总高度
        lineHeight = getLineHeight();//获取每行弹幕的最大高度
        totalLine = totalHeight / lineHeight;//总行数
    }
    ...
    private void generateItem() {
        if (messageList.size() > 0) {
            ...
            item.verticalPos = random.nextInt(totalLine) * lineHeight;//随机显示所在高度
//        itemList.add(item);
